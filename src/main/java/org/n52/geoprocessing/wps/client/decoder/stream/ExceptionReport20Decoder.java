...
import org.n52.geoprocessing.wps.client.model.OWSException;
...
public class ExceptionReport20Decoder extends AbstractElementXmlStreamReader {
    ...
    public Object readElement(XMLEventReader reader) throws XMLStreamException {
        while (reader.hasNext()) {
            ...
            if (event.isStartElement()) {
                ...
                if (start.getName().equals(OWSConstants.Elem.QN_EXCEPTION_REPORT)) {
    ...
    private ExceptionReport readExceptions(StartElement start, XMLEventReader reader) throws XMLStreamException {
        ...
        while (reader.hasNext()) {
            ...
            if (event.isStartElement()) {
                ...
                if (elem.getName().equals(OWSConstants.Elem.QN_EXCEPTION)) {
    ...
    private OWSException readException(StartElement elem, XMLEventReader reader) throws XMLStreamException {
        ...
        getAttribute(elem, OWSConstants.Attr.AN_EXCEPTION_CODE).ifPresent(exception::setExceptionCode);
        getAttribute(elem, OWSConstants.Attr.AN_LOCATOR).ifPresent(exception::setLocator);

        while (reader.hasNext()) {
            XMLEvent event = reader.nextEvent();
            if (event.isStartElement()) {
                StartElement start = event.asStartElement();
                if (start.getName().equals(OWSConstants.Elem.QN_EXCEPTION_TEXT)) {
