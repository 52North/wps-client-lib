...
// LoggerFactory.getLogger(ExecuteResponseAnalyser.class);
//
// ProcessDescriptionType processDesc;
// ExecuteDocument exec;
// Object response;
//
// public ExecuteResponseAnalyser(ExecuteDocument exec, Object response,
// ProcessDescriptionType processDesc) throws WPSClientException {
// this.processDesc = processDesc;
// this.exec= exec;
// if(response instanceof ExceptionReport){
// throw new WPSClientException("Output is not ComplexData but an Exception
// Report");
// }
...
// */
// public IData getComplexData(String outputID, Class<?> binding) throws
// WPSClientException {
...
// * @throws WPSClientException
// */
// public IData getComplexDataByIndex(int index, Class<?> binding) throws
// WPSClientException {
// ExecuteResponseDocument doc = null;
// if(response instanceof ExecuteResponseDocument){
// doc = (ExecuteResponseDocument) response;
// }else{
// throw new WPSClientException("Output cannot be determined by index since it
// is either raw data or an exception report");
// }
// OutputDataType[] outputs =
// doc.getExecuteResponse().getProcessOutputs().getOutputArray();
// int counter = 0;
// for(OutputDataType output : outputs) {
// if(output.getData().getComplexData() != null) {
// if(counter == index) {
// return this.parseProcessOutput(output.getIdentifier().getStringValue(),
// binding);
// }
// counter ++;
// }
...
// }
// }
// RuntimeException rte = new RuntimeException("No reference found in
// response");
...
// * @throws WPSClientException
// */
// private IData parseProcessOutput(String outputID, Class<?>
// outputDataBindingClass) throws WPSClientException {
...
// mimeType =
// exec.getExecute().getResponseForm().getRawDataOutput().getMimeType();
// encoding =
// exec.getExecute().getResponseForm().getRawDataOutput().getEncoding();
...
// exec.getExecute().getResponseForm().getResponseDocument().getOutputArray();
...
//
// if(mimeType==null){
// for(OutputDescriptionType tempDesc :
// processDesc.getProcessOutputs().getOutputArray()) {
...
// mimeType =
// outputDesc.getComplexOutput().getDefault().getFormat().getMimeType();
// encoding =
// outputDesc.getComplexOutput().getDefault().getFormat().getEncoding();
// schema = outputDesc.getComplexOutput().getDefault().getFormat().getSchema();
// }
//
// IParser parser =
// StaticDataHandlerRepository.getParserFactory().getParser(schema, mimeType,
// encoding, outputDataBindingClass);
// InputStream is = null;
// if(response instanceof InputStream){
// is = (InputStream)response;
// }else if (response instanceof ExecuteResponseDocument){
// ExecuteResponseDocument responseDocument = (ExecuteResponseDocument)
// response;
// OutputDataType[] processOutputs =
// responseDocument.getExecuteResponse().getProcessOutputs().getOutputArray();
...
// throw new WPSClientException("Could not fetch response from referenced URL",
// e);
// } catch (IOException e) {
// throw new WPSClientException("Could not fetch response from referenced URL",
// e);
...
// processOutput.getData().getComplexData().getDomNode().getChildNodes();
//
// Node complexDataNode = candidateNodes.getLength() > 1 ?
// candidateNodes.item(1) : candidateNodes.item(0);
//
...
// }
// }
// RuntimeException rte = new RuntimeException("Could not find suitable
// parser");
// LOGGER.error(rte.getMessage());
// throw rte;
// }
